...
func TryCreate(db *sql.DB) {
	...
	if err != nil {
		if sqlError, ok := err.(sqllite3.Error); ok {
			if sqlError.Code != 1 {
				log.Fatal(sqlError)
			}
...
func CreateEMail(db *sql.DB, emmail string) error {
...
func GetEnail(db *sql.DB, email string) (*EmailEntry, error) {
	rows, err := db.Query(`
...
type GetEnailBatchQueryParams struct {
	Page  int
	Count int
}

func GetEnailBatch(db *sql.DB, params GetEnailBatchQueryParams) ([]EmailEntry, error) {
